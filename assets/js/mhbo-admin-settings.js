...
( function ( $ ) {
	...
	function initLicenseManagement() {
		...
		$deactivateBtn.on( 'click', function ( e ) {
			e.preventDefault();
			if ( ! confirm( config.i18n?.are_you_sure || 'Are you sure?' ) ) {
				return;
			}
	...
	function initCustomFieldsRepeater() {
		...
		$addBtn.on( 'click', function () {
			...
			Object.keys( langLabels ).forEach( function ( lang ) {
				langFields +=
					...
					'<span style="width: 35px; font-weight: 600; font-size: 11px;">' +
					lang.toUpperCase() +
					':</span>' +
					'<input type="text" name="mhbo_custom_fields[' +
					index +
					'][label][' +
					lang +
					']" value="" class="widefat" style="flex: 1;"></div>';
			} );

			const html =
				'<div class="mhbo-repeater-item" style="background: #fff; border: 1px solid #ccd0d4; padding: 15px; margin-bottom: 15px; position: relative; border-radius: 4px; box-shadow: 0 1px 1px rgba(0,0,0,0.04);">' +
				...
				'<input type="text" name="mhbo_custom_fields[' +
				index +
				'][id]" value="" class="widefat" placeholder="slug" required>' +
				'</div>' +
				'<div>' +
				'<label style="display: block; font-weight: bold; margin-bottom: 5px;">Type</label>' +
				'<select name="mhbo_custom_fields[' +
				index +
				'][type]" class="widefat"><option value="text">Text</option><option value="number">Number</option><option value="textarea">Textarea</option></select>' +
				'</div>' +
				'</div>' +
				...
				langFields +
				'</div>' +
				'<div><label style="font-weight: bold;"><input type="checkbox" name="mhbo_custom_fields[' +
				index +
				'][required]" value="1"> Required Field</label></div>' +
				'</div>';
		...
		$( document ).on( 'click', '.mhbo-remove-field', function () {
			if (
				confirm(
					config.i18n?.remove_field_confirm ||
	...
	function initHolidayManagement() {
		...
		function updateList() {
			...
			if ( dates.length === 0 ) {
				$list.append(
					'<div style="color:#999; font-style:italic; padding:5px;">' +
						( config.i18n?.no_holidays ||
			...
			dates.forEach( function ( date ) {
				$list.append(
					'<div style="display:flex; justify-content:space-between; align-items:center; background:#f6f7f7; padding:6px 10px; border-radius:4px; border:1px solid #dcdcde;">' +
						...
						date +
						'</span>' +
						'<span class="dashicons dashicons-no-alt mhbo-remove-date" data-date="' +
						date +
						'" style="cursor:pointer; color:#d63638; font-size:18px;"></span>' +
						'</div>'
				);
			} );
		}
	...
	function initThemeSelection() {
		...
		$applyBtn.closest( 'form' ).on( 'submit', function ( e ) {
			if (
				$( e.originalEvent.submitter ).hasClass(
					'mhbo-apply-theme-btn'
				)
			) {
				const $btn = $( e.originalEvent.submitter );
				$btn.val( config.i18n?.applying || 'Applying Theme...' ).prop(
					'disabled',
					true
				);
			}
	...
	function initPaymentGatewayTests() {
		...
		$stripeTestBtn.on( 'click', function ( e ) {
			...
			const btn = $( this );
			const selectedMode =
				$( 'select[name="mhbo_stripe_mode"]' ).val() || 'test';

			// Send current form field values so the test works before saving
			const pkField =
				selectedMode === 'live'
					? 'mhbo_stripe_live_publishable_key'
					: 'mhbo_stripe_test_publishable_key';
			const skField =
				selectedMode === 'live'
					? 'mhbo_stripe_live_secret_key'
					: 'mhbo_stripe_test_secret_key';
			...
			$.ajax( {
				...
				data: {
					...
					mode: selectedMode,
					publishable_key:
						$( 'input[name="' + pkField + '"]' ).val() || '',
					secret_key:
						$( 'input[name="' + skField + '"]' ).val() || '',
				},
				success( response ) {
					...
					if ( response.success ) {
						if ( response.data.currency_mismatch ) {
							const $warning = $(
								'<div style="margin-top: 10px; padding: 10px; border-left: 4px solid #ffb900; background: #fff8e5; color: #444;"></div>'
							);
							const $title = $(
								'<span style="color: #d63638; font-weight: bold; display: block; margin-bottom: 5px;">⚠️ </span>'
							);
							$title.append(
								$( '<span/>' ).text(
									config.i18n?.currency_mismatch_title ||
		...
		$paypalTestBtn.on( 'click', function ( e ) {
			...
			const btn = $( this );
			const selectedMode =
				$( 'select[name="mhbo_paypal_mode"]' ).val() || 'sandbox';
			...
			const ciField =
				selectedMode === 'live'
					? 'mhbo_paypal_live_client_id'
					: 'mhbo_paypal_sandbox_client_id';
			const sField =
				selectedMode === 'live'
					? 'mhbo_paypal_live_secret'
					: 'mhbo_paypal_sandbox_secret';
			...
			$.ajax( {
				...
				data: {
					...
					mode: selectedMode,
					client_id: $( 'input[name="' + ciField + '"]' ).val() || '',
					secret: $( 'input[name="' + sField + '"]' ).val() || '',
				},
				success( response ) {
	...
	function initCacheClear() {
		const $btn = $( '#mhbo_clear_cache' );
		const $spinner = $( '#mhbo_cache_spinner' );
			...
			$.ajax( {
				...
				success( response ) {
					...
					const message = response.data
						...
						: 'Unknown error';
					$btn.after(
						'<span class="mhbo-cache-status" style="color: ' +
							color +
							'; margin-left: 10px;">' +
							message +
							'</span>'
