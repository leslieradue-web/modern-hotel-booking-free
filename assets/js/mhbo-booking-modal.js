...
( function () {
	...
	function build() {
		...
		closeBtn.setAttribute(
			'aria-label',
			( window.mhboModalI18n && window.mhboModalI18n.close ) || 'Close'
		);
		closeBtn.innerHTML = '&#10005;';
	...
	function open( title ) {
		...
		titleEl.textContent =
			title ||
			( window.mhboModalI18n && window.mhboModalI18n.bookNow ) ||
			'Book Now';
	...
	function setLoading( msg ) {
		...
		const label =
			msg ||
			( window.mhboModalI18n && window.mhboModalI18n.loading ) ||
			'Loading…';
		body.innerHTML =
	...
	function setContent( html ) {
		...
		document.dispatchEvent(
			new CustomEvent( 'mhboModalContent', { detail: { body } } )
		);
	}
	...
	function setError( msg ) {
		...
		body.innerHTML =
			'<div class="mhbo-modal-error">' + escHtml( msg ) + '</div>';
	}
	...
	document.addEventListener( 'mhboBookNow', function ( e ) {
		...
		const d = e.detail || {};
		const restBase =
			( window.mhbo_vars && window.mhbo_vars.rest_url ) ||
			( window.mhbo_calendar &&
				window.mhbo_calendar.rest_url &&
				window.mhbo_calendar.rest_url.replace( /\/[^/]+$/, '' ) ) ||
			'/wp-json/mhbo/v1';
		...
		_fetchModalEndpoint(
			...
			.then( function ( data ) {
				...
				document.dispatchEvent(
					new CustomEvent( 'mhboModalFormReady', {
						detail: { body, nonce: data.nonce, params: d },
					} )
				);
			} )
			.catch( function ( err ) {
				const label =
					( window.mhboModalI18n &&
						window.mhboModalI18n.errorLoading ) ||
		...
		return fetch( url, {
			...
			if ( ! res.ok ) {
				if ( res.status === 403 && ! _modalRetried ) {
					...
					return fetch(
						( restBase || '/wp-json/mhbo/v1' ).replace(
							/\/$/,
							''
						) + '/nonce',
						{
	...
	document.addEventListener( 'mhboBookingComplete', function ( e ) {
		...
		setLoading(
			( window.mhboModalI18n && window.mhboModalI18n.confirming ) ||
		...
		_fetchModalEndpoint(
			...
			.then( function ( data ) {
				...
				setContent( data.html );
				document.dispatchEvent(
					new CustomEvent( 'mhboConfirmationShown' )
				);
			} )
			.catch( function ( err ) {
				const label =
					( window.mhboModalI18n &&
						window.mhboModalI18n.errorConfirmation ) ||
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		...
		const token = params.get( 'mhbo_modal_booking' );
		if ( ! token ) {
			return;
		}
		const status = params.get( 'mhbo_modal_status' ) || 'confirmed';
		...
		document.dispatchEvent(
			new CustomEvent( 'mhboBookingComplete', {
				detail: { booking_token: token, status },
			} )
		);
	} );
} )();
